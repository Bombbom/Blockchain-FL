...
class NNWorker:
    ...
    def build_base(self):
        ...
        self.X = tf.placeholder("float", [None, self.num_input])
        self.Y = tf.placeholder("float", [None, self.num_classes])
        # self.X = np.reshape(self.X, (-1, 28, 28, 1))
        self.weights = {
            'w1': tf.Variable(tf.random.normal([self.num_input, self.n_hidden_1]),name="w1"),
            'w2': tf.Variable(tf.random.normal([self.n_hidden_1, self.n_hidden_2]),name="w2"),
            'wo': tf.Variable(tf.random.normal([self.n_hidden_2, self.num_classes]),name="wo")
        }
        self.biases = {
            'b1': tf.Variable(tf.random.normal([self.n_hidden_1]),name="b1"),
            'b2': tf.Variable(tf.random.normal([self.n_hidden_2]),name="b2"),
            'bo': tf.Variable(tf.random.normal([self.num_classes]),name="bo")
        }
    ...
    def get_model(self):
        ...
        '''
        varsk = {tf.compat.v1.trainable_variables()[i].name[:2]:tf.compat.v1.trainable_variables()[i].eval(self.sess) for i in range(len(tf.compat.v1.trainable_variables()))}
